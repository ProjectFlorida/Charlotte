//
//  EndlessBarChartViewController.m
//  Examples
//
...
@interface ChartPage : NSObject
@property (nonatomic, readonly) CGFloat minValue;
@property (nonatomic, readonly) CGFloat maxValue;
@property (nonatomic, strong, readonly) NSArray *values;
- (instancetype)initWithMin:(CGFloat)min max:(CGFloat)max values:(NSArray *)values;
@end

@implementation ChartPage
- (instancetype)initWithMin:(CGFloat)min max:(CGFloat)max values:(NSArray *)values
{
    self = [super init];
    if (self) {
...
@interface EndlessBarChartViewController () <CHChartViewDataSource, CHChartViewDelegate, CHBarChartViewDataSource>
...
@implementation EndlessBarChartViewController
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        _pages = @[
                   [[ChartPage alloc] initWithMin:0 max:80 values:@[@0, @10, @20, @30, @40, @50, @60]],
                   [[ChartPage alloc] initWithMin:0 max:80 values:@[@60, @50, @40, @30, @20, @10, @0]],
                   [[ChartPage alloc] initWithMin:0 max:80 values:@[@0, @10, @20, @30, @40, @50, @60]],
                   [[ChartPage alloc] initWithMin:0 max:80 values:@[@60, @50, @40, @30, @20, @10, @0]],
                   ];
        _chartView.currentPage = 79;
...
{
    ChartPage *p = self.pages[page];
    return [p.values[index] floatValue];
}
...
{
    if (page == 1) {
        CGFloat min = 0 + arc4random_uniform(20);
        CGFloat max = 80 + arc4random_uniform(20) - 10;
        ChartPage *newPage = [[ChartPage alloc] initWithMin:min max:max values:@[@0, @10, @20, @30, @40, @50, @60]];
        self.pages = [@[newPage] arrayByAddingObjectsFromArray:self.pages];
        self.chartView.currentPage = page+1;
        [self.chartView reloadData];
        [self.chartView scrollToPage:page+1 animateScrolling:NO animateRangeTransition:YES];
    }
}
